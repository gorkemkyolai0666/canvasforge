...
def sanitize_content(text: str) -> tuple[str, bool]:
    ...
    for line in lines:
        if BOX_CHAR_RE.search(line) or PRISMA_BANNER_RE.search(line):
            changed = True
            continue
        if "pris.ly/d/major-version-upgrade" in line:
            changed = True
            continue
        if re.search(r"npm i --save-dev prisma@latest", line):
            changed = True
            continue
        if re.search(r"npm i @prisma/client@latest", line):
            changed = True
            continue
        cleaned.append(line)
