...
const tiers = [
  {
    ...
    features: ['3 tahta', '5 kullanıcı', 'Temel kanban', '1 aktif sprint', 'E-posta desteği'],
  ...
  },
  {
    ...
    highlight: true,
    features: ['Sınırsız tahta', '25 kullanıcı', 'Velocity-aware sprint planner', 'Cross-board bağımlılık grafiği', 'Burndown & velocity analitiği', 'AI sprint kredileri (50/ay)', 'Öncelikli destek'],
  ...
  },
  {
    ...
    features: ['Sınırsız kullanıcı & tahta', 'SSO/SAML entegrasyonu', 'KVKK denetim raporları', 'API & webhook erişimi', 'Çoklu workspace', 'Özel hesap yöneticisi', 'White-label portal', 'SLA garantisi'],
    missing: [],
  },
];

const comparison = [
  { feature: 'Kanban tahtaları', starter: '3', pro: 'Sınırsız', enterprise: 'Sınırsız' },
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="mx-auto max-w-6xl px-4 py-16 text-center">
        <h1 className="font-display text-4xl font-extrabold tracking-tight sm:text-5xl">Şeffaf fiyatlandırma</h1>
        <p className="mx-auto mt-4 max-w-2xl text-lg text-muted-foreground">
          Jira Standard ($7.75/kullanıcı) ve Trello Premium ($5/kullanıcı) ile rekabetçi konumlandırma.
          CanvasForge Pro: <span className="font-mono text-mint-light">₺299/kullanıcı/ay</span>
        </p>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {tiers.map((t) => (
            <div key={t.id} className={t.highlight ? 'surface-card-accent p-7' : 'surface-card p-7'}>
              {t.highlight && <span className="mb-3 inline-block rounded-full bg-mint px-3 py-1 text-xs font-semibold text-primary-foreground">En popüler</span>}
              ...
              <Link href="/register" className={t.highlight
                ? 'mt-7 inline-flex w-full items-center justify-center rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground hover:bg-mint-dark'
      ...
      <footer className="mx-auto max-w-6xl px-4 py-10 text-center text-sm text-muted-foreground">
        <p>© {new Date().getFullYear()} CanvasForge · Tüm fiyatlar KDV hariçtir.</p>
      </footer>
    </div>
  );
}
