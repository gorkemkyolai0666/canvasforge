...
export default function TriagePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="font-display text-3xl font-bold">İnceleme Inbox</h1>
        <p className="text-muted-foreground">Token drift kümeleme ve otomatik inceleme önerileri</p>
      </div>
      ...
      {!loading && data && (
        <>
          <div className="grid gap-4 sm:grid-cols-3">
            <StatCard title="Bekleyen Issue" value={data.issues.length} icon={<Inbox className="h-5 w-5" />} mono />
            <StatCard title="Küme Sayısı" value={data.clusters.length} icon={<Filter className="h-5 w-5" />} mono />
            <StatCard title="Triage Kuralları" value={data.ruleCount} icon={<Sparkles className="h-5 w-5" />} mono />
          </div>
