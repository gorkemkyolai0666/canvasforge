...
async function fetchAPI(endpoint: string, options: RequestInit = {}) {
  const token = typeof window !== 'undefined' ? localStorage.getItem('canvasforge_token') : null;
  ...
  if (response.status === 401 && typeof window !== 'undefined') {
    localStorage.removeItem('canvasforge_token');
    localStorage.removeItem('canvasforge_user');
...
export const api = {
  ...
  dependencies: {
    ...
    blastRadius: (issueId: string) => fetchAPI(`/dependencies/blast-radius/${issueId}`),
  },
  team: crud('/team'),
