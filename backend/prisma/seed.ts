...
async function main() {
  const demoEmail = 'demo@canvasforge.com.tr';

  const org = await prisma.organization.upsert({
    where: { id: 'org-canvasforge-demo' },
    update: {},
    create: {
      id: 'org-canvasforge-demo',
      name: 'Tasarım Sistemleri A.Ş.',
      slug: 'tasarim-sistemleri',
      address: 'Maslak Mah. Büyükdere Cad. No:255, Sarıyer / İstanbul',
      phone: '0212 555 01 23',
      email: 'info@tasarimsistemleri.com.tr',
      planTier: 'pro',
      maxBoards: 15,
      maxUsers: 50,
      aiCredits: 87,
  ...
  const poUser = await prisma.user.upsert({
    where: { email: 'po@tasarimsistemleri.com.tr' },
    update: {},
    create: {
      email: 'po@tasarimsistemleri.com.tr',
      password: passwordHash,
      name: 'Mehmet Kaya',
      role: 'product_owner',
    },
  ...
  const devUser = await prisma.user.upsert({
    where: { email: 'dev@tasarimsistemleri.com.tr' },
    update: {},
    create: {
      email: 'dev@tasarimsistemleri.com.tr',
      password: passwordHash,
      name: 'Zeynep Demir',
      role: 'developer',
    },
  ...
  const viewerUser = await prisma.user.upsert({
    where: { email: 'izleyici@tasarimsistemleri.com.tr' },
    update: {},
    create: {
      email: 'izleyici@tasarimsistemleri.com.tr',
      password: passwordHash,
      name: 'Can Öztürk',
      role: 'viewer',
    },
  ...
  const boardProduct = await prisma.board.upsert({
    ...
    create: {
      ...
      description: 'CanvasForge çekirdek ürün kanban tahtası',
    },
  });
  ...
  const boardMobile = await prisma.board.upsert({
    ...
    create: {
      ...
      organizationId: org.id,
      name: 'Mobil Uygulama',
      description: 'iOS ve Android istemci geliştirme tahtası',
    },
  });
  ...
  const labelFeature = await prisma.label.upsert({
    ...
    update: {},
    create: { organizationId: org.id, name: 'Özellik', color: '#0ACF83' },
  });
  ...
  const issueAuth = await prisma.issue.upsert({
    ...
    create: {
      ...
      description: 'Giriş, kayıt ve profil uç noktaları',
      priority: 'high',
      status: 'done',
      storyPoints: 5,
  ...
  const issueDeps = await prisma.issue.upsert({
    ...
    create: {
      ...
      title: 'Çapraz tahta bağımlılık grafiği',
  ...
  await prisma.comment.upsert({
    ...
    create: {
      ...
      issueId: issueDeps.id,
      authorId: poUser.id,
      body: 'Kritik yol analizi için D3.js yerine mevcut canvas kütüphanesini kullanalım.',
    },
  });
  ...
  await prisma.ceremony.upsert({
    ...
    create: {
      ...
      durationMinutes: 15,
      notes: 'Zoom bağlantısı: standup.tasarimsistemleri.com.tr',
    },
  });
  ...
  await prisma.auditLog.upsert({
    ...
    create: {
      ...
      metadata: JSON.stringify({ sprintName: 'Sprint 25', boardName: 'Ürün Geliştirme' }),
    },
  });
  ...
  console.log('CanvasForge seed tamamlandı.');
