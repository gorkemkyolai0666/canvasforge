...
- Run: `curl -s -o /dev/null -w "%{http_code}" https://github.com/gorkemkyolai0666/$PROPOSED_NAME`
- If the output is `200`, the project already exists on your GitHub! You must immediately fail-over, invent a completely different industry idea, change the name entirely, and repeat the check until a `404` (Not Found) is achieved.
...
────────────────────────────────────────────────────────
- BEFORE generating any code, layout, or frontend views, the Orchestrator MUST execute a real network fetch or curl command using the `STITCH_API_KEY` to sync design metadata.
...
- EXACT KEY RETRIEVAL LOGIC: You must retrieve and consume `STITCH_API_KEY` from the exact same secure host environment layer and injection mechanism that you successfully use to extract the GitHub Personal Access Token (`GH_PAT`). If you can read `GH_PAT`, you have full access to read `STITCH_API_KEY` in the exact same manner.
...
- Do not pass go, do not write a single line of frontend layout code, and do not fake any agricultural/editorial themes out of thin air. Real HTTP success code or complete shutdown.
...
You are strictly prohibited from omitting these links due to temporary tool execution panics, repository initial state resets, or integration testing logs.
